...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, taskLog *slog.Logger) (TaskResult, error) {
	// Refuse to spawn an agent without a workspace. An empty workspace_id here
	// would make MULTICA_WORKSPACE_ID empty in the agent env, and the CLI
	// would otherwise silently fall back to the user-global config — a path
	// that can leak operations into an unrelated workspace.
	if task.WorkspaceID == "" {
