...
import { useInboxTypeLabel } from "./inbox-detail-label";
...
export function InboxPage() {
  ...
  const handleSelect = (item: InboxItem) => {
    ...
    if (!item.read) {
      markReadMutation.mutate(item.id, {
        onError: () => toast.error(t.inboxItems.failedToMarkRead),
      });
    }
  };
  ...
  const handleArchive = (id: string) => {
    ...
    archiveMutation.mutate(id, {
      onError: () => toast.error(t.inboxItems.failedToArchive),
    });
  };
  ...
  const handleMarkAllRead = () => {
    markAllReadMutation.mutate(undefined, {
      onError: () => toast.error(t.inboxItems.failedToMarkAllRead),
    });
  };
  ...
  const handleArchiveAll = () => {
    ...
    archiveAllMutation.mutate(undefined, {
      onError: () => toast.error(t.inboxItems.failedToArchiveAll),
    });
  };
  ...
  const handleArchiveAllRead = () => {
    ...
    archiveAllReadMutation.mutate(undefined, {
      onError: () => toast.error(t.inboxItems.failedToArchiveAllRead),
    });
  };
  ...
  const handleArchiveCompleted = () => {
    ...
    archiveCompletedMutation.mutate(undefined, {
      onError: () => toast.error(t.inboxItems.failedToArchiveCompleted),
    });
  };
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-auto">
          <DropdownMenuItem onClick={handleMarkAllRead}>
            ...
            {t.inboxItems.markAllAsRead}
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={handleArchiveAll}>
            <Archive className="h-4 w-4" />
            {t.inboxItems.archiveAllMenu}
          </DropdownMenuItem>
          <DropdownMenuItem onClick={handleArchiveAllRead}>
            <BookCheck className="h-4 w-4" />
            {t.inboxItems.archiveAllRead}
          </DropdownMenuItem>
          <DropdownMenuItem onClick={handleArchiveCompleted}>
            <ListChecks className="h-4 w-4" />
            {t.inboxItems.archiveCompleted}
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </PageHeader>
  ...
  const detailContent = selected?.issue_id ? (
    ...
    <div className="p-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {typeLabel(selected.type)} · {timeAgo(selected.created_at)}
      </p>
      {selected.body && (
      ...
      <div className="flex flex-col min-h-0 h-full">
        {detailContent ?? (
          <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
            ...
            <p className="text-sm">
              {items.length === 0
                ? t.inboxItems.inboxEmpty
                : t.inboxItems.selectNotification}
            </p>
          </div>
        )}
      </div>
