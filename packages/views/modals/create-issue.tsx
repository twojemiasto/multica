...
export function CreateIssueModal({ onClose, data }: { onClose: () => void; data?: Record<string, unknown> | null }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        toast.custom((toastId) => (
          <div className="bg-popover text-popover-foreground border rounded-lg shadow-lg p-4 w-[360px]">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium">{t.issue.created}</span>
            </div>
      ...
      toast.error(t.issue.createFailed);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    >
      ...
      >
        {backlogHintIssueId ? (
          <BacklogAgentHintContent
            ...
            onMoveToTodo={() => {
              updateIssueMutation.mutate(
                ...
                { onError: () => toast.error(t.issue.updateStatusFailed) },
              );
              setBacklogHintIssueId(null);
              onClose();
            }}
          ...
          <>
            <DialogTitle className="sr-only">{t.issue.newIssue}</DialogTitle>
            ...
            <div className="flex items-center justify-between px-5 pt-3 pb-2 shrink-0">
              <div className="flex items-center gap-1.5 text-xs">
                ...
                <span className="font-medium">{data?.parent_issue_id ? t.issue.newSubIssue : t.issue.new}</span>
              </div>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t shrink-0">
              ...
              <Button size="sm" onClick={handleSubmit} disabled={!title.trim() || submitting}>
                {submitting ? t.issue.creating : t.issue.createIssue}
              </Button>
            </div>
          </>
        )}
