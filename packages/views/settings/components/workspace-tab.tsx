...
export function WorkspaceTab() {
  ...
  const handleLeaveWorkspace = () => {
    if (!workspace) return;
    setConfirmAction({
      title: t.settings.leaveWorkspaceConfirm,
      description: `${t.settings.leaveWorkspace} ${workspace.name}? ${t.settings.leaveWorkspaceLoseAccess}`,
      variant: "destructive",
      onConfirm: async () => {
        setActionId("leave");
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {isSoleOwner
                    ? isSoleMember
                      ? t.settings.soleMemberCannotLeave
                      : t.settings.soleOwnerCannotLeave
