...
export function SkillsTab({
  ...
  const handleAdd = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agent.skillsAddFailed);
    } finally {
      setSaving(false);
      setShowPicker(false);
    }
  ...
  const handleRemove = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agent.skillsRemoveFailed);
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-4">
      ...
      {agent.skills.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
          ...
          <p className="mt-3 text-sm text-muted-foreground">{t.agent.skillsNoAssigned}</p>
          <p className="mt-1 text-xs text-muted-foreground">
            {t.agent.skillsNoAssignedHelp}
          </p>
      ...
      {showPicker && (
        <Dialog open onOpenChange={(v) => { if (!v) setShowPicker(false); }}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-sm">{t.agent.skillsAddDialogTitle}</DialogTitle>
              <DialogDescription className="text-xs">
                {t.agent.skillsAddDialogDesc}
              </DialogDescription>
            </DialogHeader>
